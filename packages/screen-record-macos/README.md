...
Record your screen to a self-contained MP4 file. macOS-native capture path; no third-party recorders required.
...
2. On the Home screen tile, set:
   ...
   - `screenIndex` selects the display (0 = primary, 1 = secondary, …)
3. Click **▶ Run** to start recording.
4. Click **■ Stop** to finalize the MP4. Without a clean stop the file may be unplayable.

## Requirements

- GStreamer 1.18+ with the platform-specific capture element for macOS
- Plugins: H.264 encoder (`x264enc`) + `mp4mux`
- macOS will prompt for screen-recording permission the first time you run it (System Settings → Privacy & Security → Screen Recording).
...
| `screenIndex` | `0` | 0 = primary, 1 = secondary, ... |
