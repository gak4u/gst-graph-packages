...
Record your screen to a self-contained MP4 file. Windows 10+-native capture path; no third-party recorders required.
...
2. On the Home screen tile, set:
   ...
   - `monitorIndex` selects which monitor (0 = primary)
3. Click **▶ Run** to start recording.
4. Click **■ Stop** to finalize the MP4. Without a clean stop the file may be unplayable.

## Requirements

- GStreamer 1.18+ with the platform-specific capture element for Windows 10+
- Plugins: H.264 encoder (`x264enc`) + `mp4mux`
- Windows requires desktop access; running from an elevated terminal helps if the capture surface is restricted.
...
| `monitorIndex` | `0` | 0 = primary, 1 = secondary, ... |
